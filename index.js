...
class ServerlessAmplifyPlugin {
  ...
  log(level, message) {
    if (level == "error") {
      ...
      if (this.options.v)
        console.log(chalk.green("amplify-plugin: ") + message);
    } else {
      if (process.env.SLS_DEBUG)
        console.log(chalk.blue("amplify-plugin: ") + message);
    }
  }
  ...
  async fetch(apiName, operation, parameters) {
    this.log(
      "debug",
      `fetch(${apiName}, ${operation}, ${JSON.stringify(parameters)})`
    );
    ...
    for (let i = 0; i < resources.length; i++) {
      ...
      switch (resource.ResourceType) {
        case "AWS::AppSync::GraphQLApi":
          ...
          let appSyncMetaData = await this.fetch("AppSync", "getGraphqlApi", {
            apiId: appSyncId,
          });
    ...
    for (let i = 0; i < resources.length; i++) {
      ...
      switch (resource.ResourceType) {
        case "AWS::Cognito::UserPoolClient":
          ...
          const userPoolResource = resources.filter(
            (r) =>
              r.ResourceType === "AWS::Cognito::UserPool" &&
              r.LogicalResourceId === userPoolName
          )[0];
          let result = await this.fetch(
            "CognitoIdentityServiceProvider",
          ...
          detailedResources.push(
            Object.assign({}, resource, { metadata: result })
          );
          break;
      }
    }
  ...
  getTemporaryOperationsFile(resource, schemaFile) {
    const operationsFile = path.join(
      ".serverless",
      "amplify-operations.graphql"
    );
    graphqlGenerator(schemaFile, operationsFile, {
      language: "graphql",
      maxDepth: 10,
    });
  ...
  writeConfigurationFiles(resources) {
    for (let i = 0; i < this.config.length; i++) {
      ...
      if (
        fileDetails.hasOwnProperty("type") &&
        fileDetails.hasOwnProperty("filename")
      ) {
        switch (fileDetails.type.toLowerCase()) {
          case "native":
            this.log(
              "info",
              `Writing ${fileDetails.type} file to ${fileDetails.filename}`
            );
          ...
          default:
            this.log(
              "error",
              `Invalid Amplify configuration type directive for ${JSON.stringify(
                fileDetails
              )}`
            );
            throw new Error(
        ...
        this.log(
          "error",
          `Invalid Amplify configuration directive for ${JSON.stringify(
            fileDetails
          )}`
        );
        throw new Error(
  ...
  writeNativeConfiguration(resources, fileDetails) {
    ...
    if (fileDetails.hasOwnProperty("appClient")) {
      const appClient = resources.find(
        (r) =>
      ...
      if (typeof appClient !== "undefined") {
        ...
        if (appClient.metadata.UserPoolClient.hasOwnProperty("ClientSecret")) {
          config.CognitoUserPool.Default.AppClientSecret =
            appClient.metadata.UserPoolClient.ClientSecret;
        }
      } else {
        throw new Error(
          `Invalid appClient specified: ${fileDetails.appClient}`
        );
      }
    }

    const identityPools = resources.filter(
      (r) => r.ResourceType === "AWS::Cognito::IdentityPool"
    );
    const identityPoolForUserPool = fileDetails.hasOwnProperty("appClient")
      ? identityPools.find(
          (r) =>
            r.metadata.CognitoIdentityProviders &&
            r.metadata.CognitoIdentityProviders.some(
              ({ ClientId }) =>
                ClientId === config.CognitoUserPool.Default.AppClientId
            )
        )
      : undefined;
    ...
    const appSync = resources.find(
      (r) => r.ResourceType === "AWS::AppSync::GraphQLApi"
    );
    ...
    let s3buckets = resources.filter(
      (r) =>
        r.ResourceType === "AWS::S3::Bucket" &&
        r.LogicalResourceId !== "ServerlessDeploymentBucket"
    );
    if (s3buckets.length > 0) {
      let userFiles = fileDetails.hasOwnProperty("s3bucket")
        ? s3buckets.find((r) => r.LogicalResourceId === fileDetails.s3bucket)
        : s3buckets[0];
    ...
    if (apigw.length > 0) {
      ...
      apigw.forEach((v) => {
        apiRecords[v.LogicalResourceId] = {
          ...
          }.execute-api.${this.provider.getRegion()}.amazonaws.com/${this.provider.getStage()}`,
          Region: this.provider.getRegion(),
        };
      });
  ...
  getJavaScriptConfiguration(resources, fileDetails) {
    ...
    if (fileDetails.hasOwnProperty("appClient")) {
      ...
      if (typeof appClient !== "undefined") {
        config.aws_cognito_region = appClient.metadata.UserPoolClient.UserPoolId.split(
          "_"
        )[0];
        config.aws_user_pools_id = appClient.metadata.UserPoolClient.UserPoolId;
        config.aws_user_pools_web_client_id =
          appClient.metadata.UserPoolClient.ClientId;

        if (appClient.metadata.UserPoolClient.hasOwnProperty("ClientSecret")) {
          config.aws_user_pools_web_client_secret =
            appClient.metadata.UserPoolClient.ClientSecret;
        }
      } else {
        throw new Error(
    ...
    const identityPoolForUserPool = fileDetails.hasOwnProperty("appClient")
      ? identityPools.find(
          (r) =>
            r.metadata.CognitoIdentityProviders &&
            r.metadata.CognitoIdentityProviders.some(
              ({ ClientId }) => ClientId === config.aws_user_pools_web_client_id
            )
        )
      : undefined;
    const identityPool = identityPoolForUserPool || identityPools[0];
    if (typeof identityPool !== "undefined") {
      if (!config.hasOwnProperty("aws_cognito_region")) {
        config.aws_cognito_region = identityPool.PhysicalResourceId.split(
          ":"
        )[0];
      }
  ...
  writeJavaScriptConfiguration(resources, fileDetails) {
    ...
    let config_body = `const awsmobile = ${stringify(config, {
      pretty: true,
      space: 4,
    })};`;
    ...
    this.writeConfigurationFile(
      fileDetails.filename,
      [config_header, config_body, config_footer].join("\n")
    );
  }
  ...
  writeTypeScriptConfiguration(resources, fileDetails) {
    ...
    let config_body = `const awsmobile: IAWSAmplifyConfiguration = ${stringify(
      config,
      { pretty: true, space: 4 }
    )};`;
    let config_footer = "\nexport default awsmobile;\n";
  ...
  writeGraphQLOperations(resources, fileDetails) {
    ...
    if (resource) {
      ...
      throw new Error(
        `No GraphQL API found - cannot write ${fileDetails.filename} file`
      );
    }
  }
  ...
  writeAppSyncAPI(resources, fileDetails) {
    ...
    if (resource) {
      ...
      const graphqlFile = path.resolve(
        this.getTemporaryOperationsFile(resource, schemaFile)
      );
